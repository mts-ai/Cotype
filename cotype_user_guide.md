...
2. Заполните тело запроса.<br> **Обязательные параметры**: `model` и `messages`. В `model` необходимо указать ID вашей модели. Получить ID модели можно с помощью запроса `GET /models`, подробное описание смотрите в разделе [3.2 - GET /models](##32).<br> В `messages` необходимо указать "role" и "content". У атрибута "role" может быть одно из двух значений:
