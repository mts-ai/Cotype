...
2. *Однотокеновая авторизация*
   ...
   Вид авторизации можно установить через переменные окружения. Подробности смотрите в разделе [Проверка и конфигурация ENV-файла](#проверка-и-конфигурация-env-файла).
...
Подробнее о переменных смотрите в разделе [Переменные журналирования](#переменные-журналирования).
...
При необходимости, сконфигурируйте переменные. Подробнее о переменных рассказано в разделе [Проверка и конфигурация ENV-файла](#проверка-и-конфигурация-env-файла).
...
Для подключения к БД укажите переменные окружения. Подробнее о переменных смотрите в разделе [Переменные базы данных](#переменные-базы-данных).
...
1. С помощью API-запроса `GET/health`. Запрос подробно описан в разделе [GET/health](#gethealth).
...
Пример вызова модели с использованием тулов смотрите в разделе [Пример выполнения запроса POST/v1/chat/completions с функцией tool-calling](#пример-выполнения-запроса-postv1chatcompletions-с-функцией-tool-calling).
...
|model (обязательный)| string | ID | ID модели, к которой вы обращаетесь. Вы можете узнать список доступных вам  моделей через запрос `GET/v1/models`. Подробнее в  разделе [GET/v1/models](#getv1models). |
...
2. Заполните тело запроса.<br> **Обязательные параметры**: `model` и `messages`. В `model` необходимо указать ID вашей модели. Получить ID модели можно с помощью запроса `GET /v1/models`, подробное описание смотрите в разделе [GET /v1/models](#getv1models).<br> В `messages` необходимо указать "role" и "content". У атрибута "role" может быть одно из двух значений:
...
1. Укажите **Bearer Token**. 
   ...
   При активной однотокеновой авторизации токен одинаковый у всех пользователей. При пользовательской авторизации выдаются индивидуальные токены. Подробнее о типах авторизации и их настройке в разделе [Переменные авторизации](#переменные-авторизации).

2. Заполните тело запроса. <br>
**Обязательные параметры**: `model` и `prompt`. В `model` необходимо указать ID вашей модели. Получить ID модели можно с помощью запроса `GET/v1/models`, подробное описание смотрите в разделе [GET/v1/models](#getv1models). <br>
Пример curl-запроса для выполнения запроса из командной строки:
