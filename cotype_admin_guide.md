...
2. *Однотокеновая авторизация*
   ...
   Вид авторизации можно установить через переменные окружения. Подробности смотрите в разделе [Проверка и конфигурация ENV-файла](#check-and-configure).
...
Подробнее о переменных смотрите в разделе [Переменные журналирования](#переменные_журналирования).
...
При необходимости, сконфигурируйте переменные. Подробнее о переменных рассказано в разделе [Проверка и конфигурация ENV-файла](#проверка_и_конфигурация_env_файла).
...
Для подключения к БД укажите переменные окружения. Подробнее о переменных смотрите в разделе [Переменные базы данных](#переменные_базы_данных).
...
1. С помощью API-запроса `GET/health`. Запрос подробно описан в разделе [GET/health](#get_health).
...
Описание параметров для запроса `POST/v1/chat/completions` смотрите в разделе [POST/v1/chat/completions](#postv1chatcompletions). <br>
Пример вызова модели с использованием тулов смотрите в разделе [Пример выполнения запроса POST/v1/chat/completions с функцией tool-calling](#пример_выполнения_запроса_postv1chatcompletions_с_функцией_tool_calling).
...
1. Укажите **Bearer Token**.<br> При активной однотокеновой авторизации токен одинаковый у всех пользователей. При пользовательской авторизации выдаются индивидуальные токены. Подробнее о типах авторизации и их настройке смотрите в разделе [Переменные авторизации](#переменные-авторизации)
2. Заполните тело запроса.<br> **Обязательные параметры**: `model` и `messages`. В `model` необходимо указать ID вашей модели. Получить ID модели можно с помощью запроса `GET /v1/models`, подробное описание смотрите в разделе [GET /v1/models](#get_v1_models).<br> В `messages` необходимо указать "role" и "content". У атрибута "role" может быть одно из двух значений:
...
1. Укажите **Bearer Token**. 
   ...
   При активной однотокеновой авторизации токен одинаковый у всех пользователей. При пользовательской авторизации выдаются индивидуальные токены. Подробнее о типах авторизации и их настройке в разделе [Переменные авторизации](#переменные_авторизации).

2. Заполните тело запроса. <br>
**Обязательные параметры**: `model` и `prompt`. В `model` необходимо указать ID вашей модели. Получить ID модели можно с помощью запроса `GET/v1/models`, подробное описание смотрите в разделе [GET/v1/models](#get_v1_models). <br>
Пример curl-запроса для выполнения запроса из командной строки:
